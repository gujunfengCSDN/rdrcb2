...
var Scroll = {
  ...
  scrollByStep: function scrollByStep(step,v) {
    window.scrollByStepStop=false;
    return this.each(function animate() {
      var el = this;
      ...
      var times = 0;
      var lastStep=step;
      var lastStepDecelerateV=v;
      var onScrollDirection=0;
      if(step>0)
      {
        onScrollDirection=1;
      }
      ...
      function render(time) {
        ...
        if(window.scrollByStepStop)
        {
          done = true;
        }
        if(onScrollDirection==0)
        {
          lastStep-=lastStepDecelerateV;
          if(lastStep>=0)
          {
            done = true;
          }
        ...
        var newTop=currentTop-lastStep;
        if (newTop<0) {
          newTop=0;
          done=true;
        }
        ...
        var Autocomplete = function (params) {
            ...
            if (a.params.openIn !== 'dropdown') {
                ...
                var searchbarHTML =
                    ...
                    '</form>' +
                    '<div class="searchbar-overlay"></div>';
                var pageContent = a.params.pageContent || '';
                var pageHTML =
                    ...
                    '<div class="pages">' +
                        '<div data-page="' + pageName + '" class="page autocomplete-page ' + noNavbar + ' ' + noToolbar + '">' +
                            ...
                            '<div class="page-content" style="background-color: white;padding-bottom: 0px;">' +
                                ...
                                pageContent+
                                '<div class="list-block autocomplete-found autocomplete-list' + a.id + (formTheme ? 'theme-' + formTheme : '') + '" style="margin: 0">' +
                                    '<ul></ul>' +
                                '</div>' +
                                '<div class="list-block autocomplete-not-found"  style="margin: 0">' +
                                    '<ul><li class="item-content"><div class="item-inner"><div class="item-title">' + a.params.notFoundText + '</div></div></li></ul>' +
                                '</div>' +
                                // '<div class="list-block autocomplete-values" style="margin: 0">' +
                    //     '<ul></ul>' +
                    // '</div>' +
                            '</div>' +
                        '</div>' +
            ...
            a.pageInit = function (e) {
                ...
                function onSource(query) {
                    ...
                    a.params.source(a, query, function(items) {
                        ...
                        var limit = a.params.limit ? Math.min(a.params.limit, items.length) : items.length;
                        var itemsHTML = '<div style="margin:8px;font-size:16px">搜索到'+count+'条</div>';
                        if(count > limit){
                            itemsHTML = '<div style="margin:8px;font-size:16px">搜索到'+count+'条(显示前'+limit+'条)</div>';
                        }
                        a.items = items;
                        for (i = 0; i < limit; i++) {
                            var selected = false;
                            ...
                            itemsHTML += a.itemTemplate({
                                ...
                                count:count,
                                limit:limit,
                                zIsDepartment:items[i].zIsDepartment,
                                zSex:items[i].zSex,
                                zPhone:items[i].zPhone,
                                zEmail:items[i].zEmail,
        ...
        var Picker = function (params) {
            ...
            p.initPickerCol = function (colElement, updateItems) {
                ...
                col.calcSize = function () {
                    ...
                    itemHeight = col.items[0]!==undefined?col.items[0].offsetHeight:0;
